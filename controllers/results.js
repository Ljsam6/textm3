module.exports = function (async, Club, Users) {
    return {
        ...
        postResults: function (req, res) {
            async.parallel([
                function (callback) {
                    const regex = new RegExp((req.body.stream), 'gi');

                    Club.find({ '$or': [{ 'stream': regex }, { 'class': regex }] }, (err, result) => {
                        callback(err, result);
                    });
                }
            ], (err, results) => {
                ...
                res.render('results', { title: 'TextMe- Results', user: req.user, chunks: dataChunk });
            })
        },
        ...
        viewMembers: function (req, res) {
            async.parallel([
                ...
                res.render('members', { title: 'TextMe - Members', user: req.user, chunks: dataChunk });
            })
        },
