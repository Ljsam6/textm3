    ...
    return {
        ...
        getInterestPage: function (req, res) {
            async.parallel([
                ...
                res.render('user/interest', { title: 'TextMe - Interests', user: req.user, data: result1, chat: result2 });
            });
        },
            ...
            async.parallel([
                function (callback) {
                    if (req.body.favClub) { 
                        Users.update({
                            '_id': req.user._id,
                            'favGroup.groupName': { $ne: req.body.favClub }
                        }, 
                            {
                                $push: {
                                    favGroup: {
                                        groupName: req.body.favClub
                                    }
                                }
                            }, (err, result1) => {
            ...
            async.parallel([
                function (callback) {
                    if (req.body.favPlayer) {
                        Users.update({
                            '_id': req.user._id,
                            'hobbies.hobbieName': { $ne: req.body.favPlayer }
                        },
                            {
                                $push: {
                                    hobbies: {
                                        hobbieName: req.body.favPlayer
                                    }
                                }
                            }, (err, result2) => {
